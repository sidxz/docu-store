...
class TestReportGeneration:
    ...
    def test_latex_with_significance(self):
        ...
        report.config_results[1].significance_vs_baseline["precision_at_5"] = (
            SignificanceResult(statistic=-0.3, p_value=0.001, method="paired_bootstrap")
        )
        latex = format_latex_table(report)
    ...
    def test_ablation_table_with_ci(self):
        report = self._sample_report()
        report.config_results[0].confidence_intervals["precision_at_5"] = (
            ConfidenceInterval(mean=0.8, lower=0.75, upper=0.85)
        )
